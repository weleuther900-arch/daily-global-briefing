...
每条入选事件都要同时说明核心判断、大白话讲解、相关影响和判断边界；“三分钟商业思考”是独立的经营决策练习，包含决策角色、二选一行动和三个待核验变量，不是新闻复述或标准答案。
...
当前来源注册表包含 **53 个入口、9 个覆盖组**，横跨官方机构、监管部门、国际组织、央行、公司发布页、重点科技公司的投资者关系财报页、GitHub 以及权威媒体。系统优先用原始和官方材料支撑事实；媒体用于发现、交叉核对和补足背景。具体覆盖、筛选标准和已知盲区见 [来源与覆盖](docs/来源与覆盖.md)。
...
推荐部署路径不需要服务器。若你的每日内容属于私人资料，请将代码部署到**私有仓库**运行；公开仓库应只用于分发代码、文档与示例。此工作流不会再上传晨报 Artifact 或 Actions 缓存，但公开仓库的运行日志仍会保留最少的状态信息。

1. 在 GitHub 创建自己的部署副本：私人内容选择私有仓库；只做公开测试或二次开发时可以直接 Fork。
...
完整的逐步操作、私有部署路径、首次测试、时间表和故障排查见 [部署指南](docs/部署指南.md)。GitHub Actions 定时任务必须位于默认分支；公开仓库连续 60 天没有活动时，GitHub 也可能自动停用定时工作流，重新启用即可。
...
- `logs/runtime.jsonl`：运行阶段、完成状态和错误摘要的实时追加日志（不含密钥或邮件正文）
- `logs/project-changes.jsonl`：由 `scripts/watch-project.cjs` 记录的本地文件变动日志

在本机持续记录文件变动：`node scripts/watch-project.cjs`。
